document.addEventListener("DOMContentLoaded", () => {
    ...
    queryList.forEach((query, index) => {
      ...
      card.innerHTML = `
        <div class="card-body">
          ...
          <p class="card-text"><code>${query.query}</code></p>
          <p class="card-text"><small class="text-muted">${query.description}</small></p>
          <button class="btn btn-success" onclick="runQuery(${index})">Rodar Query</button>
        </div>
      `;
  ...
  window.runQuery = (index) => {
    ...
    .then(data => {
      alert(`Resultado da Query: ${JSON.stringify(data)}`);
    })
    .catch(error => {
